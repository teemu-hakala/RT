...
static void set_colours(t_texture *texture, t_parser *parser, int face)
{
	...
	{
		parser->c++;
		set_colours(texture, parser, face);
	}
	else if (!find_matching_bracket(parser))
		handle_errors("face syntax error");
}

static void set_default_colours(t_texture *texture, int face)
{
...
static void set_up_and_down(t_texture *texture, t_parser *parser)
{
	if (ft_strncmp(&parser->string[parser->c], "\"up\"", 4) == 0)
	{
		...
		set_default_colours(texture, FACE_UP);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_UP);
	}
	else if (ft_strncmp(&parser->string[parser->c], "\"down\"", 6) == 0)
	{
		...
		find_colon(parser);
		find_open_bracket(parser);
		set_default_colours(texture, FACE_DOWN);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_DOWN);
	}
	else
...
static void set_left_and_right(t_texture *texture, t_parser *parser)
{
	if (ft_strncmp(&parser->string[parser->c], "\"left\"", 6) == 0)
	{
		...
		find_colon(parser);
		find_open_bracket(parser);
		set_default_colours(texture, FACE_LEFT);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_LEFT);
	}
	else if (ft_strncmp(&parser->string[parser->c], "\"right\"", 7) == 0)
	{
		...
		find_colon(parser);
		find_open_bracket(parser);
		set_default_colours(texture, FACE_RIGHT);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_RIGHT);
	}
	else
		set_up_and_down(texture, parser);
}

static void	set_face(t_texture *texture, t_parser *parser)
{
	parser->c += ft_clear_whitespace(&parser->string[parser->c]);
	if (ft_strncmp(&parser->string[parser->c], "\"front\"", 7) == 0)
	{
		...
		find_colon(parser);
		find_open_bracket(parser);
		set_default_colours(texture, FACE_FRONT);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_FRONT);
	}
	else if (ft_strncmp(&parser->string[parser->c], "\"back\"", 6) == 0)
	{
		...
		find_colon(parser);
		find_open_bracket(parser);
		set_default_colours(texture, FACE_BACK);
		if (find_matching_bracket(parser))
			return ;
		set_colours(texture, parser, FACE_BACK);
	}
	else
