...
/*   Updated: 2022/10/18 12:54:32 by deelliot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_mtx submatrix(t_mtx *mtx, uint32_t size, uint32_t row, uint32_t col)
{
	t_mtx		new_matrix;
	uint32_t	row_new;
	uint32_t	col_new;
	...
	while (row_old <= size - 1)
	{
		...
		while (col_old <= size - 1)
		{
			...
			new_matrix.array[(size - 1) * row_new + col_new] = \
			mtx->array[size * row_old + col_old];
			col_old++;
			col_new++;
		}
		row_old++;
...
static t_fl	matrix_minors(t_mtx *mtx, uint32_t row, uint32_t col, uint32_t size)
{
	t_mtx	temp;
	t_fl	minor;
...
static t_fl	matrix_cofactor(t_mtx *mtx, uint32_t row, uint32_t col, uint32_t size)
{
	t_fl	cofactor;
	t_fl	minor;
...
t_fl	determinant(t_mtx *mtx, uint32_t size)
{
	...
	else
	{
		while (col < size)
		{
			det = det + mtx->array[size * 0 + col] * \
			matrix_cofactor(mtx, 0, col, size);
			col++;
		}
	}
	return (det);
