...
/*   Updated: 2022/10/14 11:27:45 by deelliot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sphere_intersection(t_ray *ray, t_sphere *sphere, t_intersections *array)
{
	t_fl discriminant;
	t_fl a;
	t_fl b;
	...
	// array->intersections[0].shape = ??  how do we link the sphere here
	array->intersections[1].time = (-b + sqrt(discriminant)) / (2 * a);
	// array->intersections[0].shape =
}
...
t_object sphere(t_tuple *origin, t_transform *transform, t_tuple *colour)
{
	return ((t_object)
	{
		...
		{
			.origin = (t_tuple){origin->tuple.units.x,
			...
			.transform = (t_transform){transform->translation,
				transform->rotation,
				transform->scale},
			.colour = (t_tuple){colour->tuple.colour.a,
