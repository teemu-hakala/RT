...
void	striped_at(t_pattern *pattern, t_material *material, t_tuple *point)
{
	if (fmod(floor(point->tuple.units.x), 2) == 0)
		material->final_colour = pattern->colour.a;
	else
		material->final_colour = pattern->colour.b;
}
...
void	gradient_at(t_pattern *pattern, t_material *material, t_tuple *point)
{
	...
	material->final_colour = tuple_add(pattern->colour.a, \
	tuple_scale(tuple_sub(pattern->colour.b, \
	pattern->colour.a), x_fraction));
}
