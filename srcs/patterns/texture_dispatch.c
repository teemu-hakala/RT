...
t_tuple	get_appearance_colour(t_world *world, t_tuple *p, t_hit *hit)
{
	...
	if (hit->intersection.appearance.pattern.type > 0)
	{
		...
		colour = pattern_at(&hit->intersection.appearance.pattern, &pattern_space);
	}
	else
	{
		pattern_space = transform_point(p, &hit->intersection.transform, \
		...
		{
			hit->intersection.appearance.texture.face = face_from_point(&pattern_space);
		}
		hit->intersection.appearance.texture.map = hit->intersection.f(&pattern_space);
